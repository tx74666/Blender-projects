...
def _retarget_custom_shape_frame(pose_bone, transform):
    """Change a display-only transform reference without a visible pose pop."""
    ...
    next_local = next_matrix.inverted() @ current_frame
    if not _matrix_is_finite(next_local):
        raise LimbIKError(
            f"Control '{pose_bone.name}' cannot derive a finite Auto Align display frame."
        )
    translation, quaternion, scale = next_local.decompose()
    ...
    if any(
        ...
        raise LimbIKError(
            f"Control '{pose_bone.name}' cannot derive a finite Auto Align visual offset."
        )
    rebuilt_local = (
    ...
    magnitude = max(
        1.0,
        *(abs(float(next_local[row][column])) for row in range(4) for column in range(4)),
    )
    residual = max(
        abs(float(next_local[row][column] - rebuilt_local[row][column]))
        for row in range(4)
        for column in range(4)
    )
    if residual > 2.0e-5 * magnitude:
        raise LimbIKError(
            f"Control '{pose_bone.name}' has display shear that Auto Align cannot preserve safely."
        )
    ...
    try:
        ...
        actual_residual = max(
            abs(float(current_frame[row][column] - actual_frame[row][column]))
            for row in range(4)
            for column in range(4)
        )
        actual_magnitude = max(
            1.0,
            *(abs(float(current_frame[row][column])) for row in range(4) for column in range(4)),
        )
...
def _draw_direct_pole_guides():
    ...
    try:
        ...
        vertices = _pole_guide_line_vertices(segments)
        if not vertices:
            return
...
def _create_constraints_and_shapes(
    ...
    *,
    ...
    for plan in plans:
        ...
        # Auto Align keeps the real Target translation-only while the Hand or
        # Foot inherits the live solved Forearm/Shin orientation.  Use that
        # evaluated end solely as the Custom Shape display frame so the visible
        # control follows its wrist/ankle normal without introducing a
        # Target -> solver -> end -> Target cycle.
        if plan.auto_align:
...
def _set_auto_align_selected_target(context, armature, settings, enabled=None):
    ...
    polling handler, or transform writes.  The same live end bone becomes the
    display-only Custom Shape frame, so both wrist and sole controls follow the
    evaluated end normal.  Returning to Manual first aligns the Target while
    Auto is still live, then unmutes without a pose pop.
    """
...
class CHARACTERDESIGNER_OT_limb_ik_auto_align_target(Operator):
    ...
    bl_description = "Toggle continuous natural alignment for every generated Arm and Leg"
    bl_options = {"REGISTER", "UNDO"}
...
class CHARACTERDESIGNER_OT_limb_ik_reset_control_visual(Operator):
    ...
    def execute(self, context):
        ...
        try:
            ...
            auto_target = (
                pose_bone.bone.get(ROLE_KEY) in {"HAND_IK", "FOOT_IK"}
                and pose_bone.bone.get(AUTO_ALIGN_KEY, False) is True
            )
            if before_transform is None or not auto_target:
                _apply_control_visual_state(pose_bone, default)
            else:
                # Generated Target defaults are authored in the Target's own
                # Manual frame.  Auto uses the evaluated Hand/Foot as a
                # display-only frame, so reinterpret that canonical default
                # there instead of applying Target-local vectors as end-local
                # vectors and visibly rotating/scaling the controller.
                pose_bone.custom_shape_transform = None
                _apply_control_visual_state(pose_bone, default)
                _retarget_custom_shape_frame(pose_bone, before_transform)
