...
Version 0.31.1 keeps each Direct Pre-Roll Pole as one visual arrow. The
existing `WGT_Randy_PoleArrow` custom shape is the sole arrowhead, and the
selected-only GPU overlay contributes only the line from the real bend joint
to that displayed tip. The custom head's base plane remains perpendicular to
the shaft through the Pole's own axis, so no second view-facing head, helper
bone, constraint, or persistent scene object is created.
...
Direct Pre-Roll elbow and knee guides are shown only while their Pole control
is selected in Pose Mode. Viewport, Overlay, Bones, bone, and Bone Collection
visibility are respected. Stable rigs keep their existing constrained Pole
guide.
...
the sole, and away from the body centerline. Old rigs that share the 46-point
`FOOT` widget remain removable; successful Rebuild upgrades them, while failed
Rebuild restores the old shared object, mesh, assignments, and display state.
...
Foot Targets now fit their flat shoe outline from evaluated, currently visible
geometry. Deform weights establish the trusted left/right foot seed; nearby
separate footwear is included, while hidden backup meshes and unrelated distant
objects are ignored. The outline receives a small perimeter margin, is placed
wholly below the lowest captured sole, and is clamped away from the character
centerline. Direct Pre-Roll remains exactly two generated bones per limb:
Target + Pole.
...
2. Click **Build IK** to create only the limb selected in the dropdown, or
   **Build All** to complete every populated Arm and Leg together in one atomic
   Undo step. Direct adds only Target + Pole per limb; Stable adds its hidden
   roll-preserving graph and animator helpers. A partly filled mapping cancels
   Build All before any generated data is written.
