...
def _apply_direct_preroll(context, armature, plans, transaction):
    ...
    for plan, _result, _saved_rest in computations:
        if plan.preserve_pole_angle:
            ...
            prepared.append(replace(rebuilt, rig_id=plan.rig_id))
...
def _validate_inventory(armature):
    ...
    if not armature_id:
        if generated or records or SCHEMA_KEY in armature.data or DIRECT_REST_KEY in armature.data:
    ...
    if not generated and not records:
        if DIRECT_REST_KEY in armature.data:
            raise LimbIKError("Orphaned Direct Pre-Roll Rest data was found on this Armature.")
...
def _create_constraints_and_shapes(context, armature, armature_id, plans, transaction, *, schema=CURRENT_SCHEMA, create_master=False):
    ...
    for plan in plans:
        ...
        if schema == LEGACY_SCHEMA:
            ...
            if plan.chain.kind == "ARM":
                ...
                foot_length = max(
                    (
                        Vector(armature.pose.bones[plan.chain.end].tail)
                        - Vector(armature.pose.bones[plan.chain.end].head)
                    ).length,
                    chain_length * 0.12,
                )
                set_shape(
                    ...
                    widgets["LEG"],
                    foot_length,
                    translation=(0.0, foot_length * 0.12, -foot_length * 0.08),
                    mirror_x=plan.chain.side == "R",
                )
        else:
            line = armature.pose.bones[plan.line_name]
            display = armature.pose.bones[plan.display_name]
...
def _build_plans(context, armature, plans, *, schema=CURRENT_SCHEMA, desired_source_pose=None):
...
def _direct_source_dependency_problems(armature, chains, *, owned_constraints=()):
    ...
    animated_names = {name for chain in chains for name in chain}
    if not rest_names:
        return []
    allowed_children = {
        pair
        ...
        for pose_bone in rig_object.pose.bones:
            for constraint in pose_bone.constraints:
                ...
                if pointer_key in owned_constraints:
                    continue
                if rig_object is armature and pose_bone.name in rest_names:
                    problems.append(
