...
def _calculate_object(obj, depsgraph):
    ...
    for side, record in records.items():
        ...
        if record["rest"] != _rest_signature(armature, record["chain"]):
            raise ForearmTwistError("Bone Rest frames changed; use Recalibrate or Sync Other Arm to rebind unchanged loops.")
...
def _current_record(obj, arm, rig, side, record, records):
    """An explicit recalibration/sync can rebind unchanged loops after Rebuild."""
...
def _prepare_mirror(obj, source_side, source_record=None):
    records = _records(obj)
...
def start_test(context, obj, side="L", *, symmetry=False):
    ...
    if symmetry:
        ...
        if other and other["rest"] != prepared[other_side]["rest"]:
            raise ForearmTwistError("Other arm Rest changed; use Sync Other Arm to rebind both, then recalibrate.")
    old_json = obj.get(RECORD_KEY)
    old_index = obj.active_shape_key_index
...
class CHARACTERDESIGNER_OT_forearm_twist_start(Operator):
    ...
    def execute(self, context):
        try:
            settings = context.window_manager.character_designer_forearm_twist
            start_test(context, context.object, settings.side, symmetry=settings.symmetry)
            return {"FINISHED"}
        except Exception as exc:
...
class CHARACTERDESIGNER_OT_forearm_twist_mirror(Operator):
    bl_idname = "character_designer.forearm_twist_mirror"
    bl_label = "Sync Other Arm"
    bl_description = "Copy this arm's loop shares to its mirrored arm, keeping both hand poses"
    bl_options = {"REGISTER", "UNDO"}

    def execute(self, context):
        try:
            mirror_calibration(context, context.object, context.window_manager.character_designer_forearm_twist.side)
            return {"FINISHED"}
        except Exception as exc:
            self.report({"ERROR"}, str(exc))
            return {"CANCELLED"}
...
class CHARACTERDESIGNER_PT_forearm_twist(Panel):
    ...
    def draw(self, context):
        ...
        if _SESSION:
            ...
            if _SESSION.get("symmetry"):
                layout.label(text="Confirm syncs shares to the other arm.", icon="MOD_MIRROR")
            ...
            if record:
                ...
                layout.operator("character_designer.forearm_twist_mirror", icon="MOD_MIRROR")
...
FOREARM_TWIST_CLASSES = (
    ...
    CHARACTERDESIGNER_OT_forearm_twist_mirror,
