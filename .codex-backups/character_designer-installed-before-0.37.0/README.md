...
Version 0.36.0 adds **Mirror Calibration**, enabled by default in Forearm
Twist. Confirming either arm copies its loop shares to the corresponding
loops of the other arm. **Sync Other Arm** also applies an existing calibration
without entering a test. Both hands retain independent control poses, and
each side uses its own weights and correction region. Pairing checks mirrored
Basis geometry, loop connectivity and bone Rest endpoints; an ambiguous or
asymmetric pair is refused before the test starts. Turn Mirror Calibration
off to calibrate such arms individually. Cancel leaves the other arm unchanged.
If Rebuild changes the bone Rest frames, correction pauses. Recalibrate or
Sync Other Arm recaptures the current axis and support while retaining shares
by exact loop vertex membership. Changed topology or different loops require
removing the old calibration. Rebinding never changes the rig itself.
...
Version 0.35.0 adds **Rig > Forearm Twist (Prototype)**. Select the bound body
Mesh in Object Mode, choose Left/Right Arm, and press **Start 90° Test**. The
...
poses resume automatically. Remove and recapture after topology changes;
use Recalibrate or Sync Other Arm after Rest changes when the loops are intact.
...
The production `X.blend` is not migrated or saved by this release.
