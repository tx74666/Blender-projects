...
`addons/character_designer`. Version 0.34.2 fixes the **Auto Align** display
...
`.blend`. Version 0.21.0 adds a separate **Weight Flow**
panel: in Weight Paint, select a chain, closed loop, or surface region plus
exactly two Deform Pose bones, then preview a topology-aware transition. It
redistributes only the two groups' original per-vertex budget; unrelated groups,
unselected vertices, modifiers, parenting, and Deform flags remain unchanged.
Linear, Smooth, and Sharp profiles and 0–1 Strength update from one immutable
Preview snapshot; OK is one Undo step and Esc restores exactly. Version 0.19.0
