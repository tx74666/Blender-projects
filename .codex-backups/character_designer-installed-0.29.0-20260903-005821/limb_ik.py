...
def _hydrate_pole_directions(settings, armature):
    """Load saved generated directions, or the anatomical local-axis defaults.
    ...
    for kind in KINDS:
        for side in SIDES:
            value = DEFAULT_POLE_DIRECTIONS[(kind, side)]
            ...
            if (
                ...
                try:
                    ...
                    pass
            setattr(settings, _pole_direction_field(kind, side), value)
...
class CHARACTERDESIGNER_OT_limb_ik_analyze(Operator):
    ...
    def execute(self, context):
        ...
        try:
            ...
            if armature_changed:
                _hydrate_pole_directions(settings, armature)
...
class CHARACTERDESIGNER_OT_limb_ik_default_pole_direction(Operator):
    ...
    def execute(self, context):
        ...
        if kind not in KINDS or side not in SIDES:
            ...
            return {"CANCELLED"}
        setattr(settings, _pole_direction_field(kind, side), DEFAULT_POLE_DIRECTIONS[(kind, side)])
        message = f"Reset {side} {kind.title()} Pole Direction to the Armature-local default."
        _set_status(settings, "SUCCESS", message)
        self.report({"INFO"}, message)
        return {"FINISHED"}
