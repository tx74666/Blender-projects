...
Every dropdown limb now has an explicit Armature-local **Pole Direction XYZ**
and a **Default Direction** button. Both arms default behind the character at
`+Y`; both knees default forward at `-Y`. Magnitude is normalized, so **Pole
Distance** remains the only distance control. Zero, non-finite, or chain-parallel
...
1. Make the intended source Armature active in Object or Pose Mode, open the
   **Rig** page, and click **Analyze Rig**. Choose Left/Right Arm or Leg in the
   ...
   anatomical arm-behind (`+Y`) or knee-forward (`-Y`) placement.
