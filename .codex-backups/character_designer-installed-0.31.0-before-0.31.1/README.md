...
Direct Pre-Roll elbow and knee guides are now one GPU-only arrow drawing shown
only while its Pole control is selected in Pose Mode. The shaft runs from the
real bend joint to the Pole, and the closed arrowhead's base stays perpendicular
to that shaft in the current view. Viewport, Overlay, Bones, bone, and Bone
Collection visibility are respected. The complete guide is one display batch;
it creates no VIS/MCH/helper bone or persistent scene object. Stable rigs keep
their existing constrained Pole guide.
