...
POLE_GUIDE_COLOR = (0.12, 0.78, 0.92, 0.82)
...
def _is_direct_preroll_schema(schema):
    return schema == DIRECT_PREROLL_SCHEMA
    ...
    for armature in tuple(view_layer.objects):
        ...
        try:
            ...
            if (
                ...
                or not armature_id
                or armature.data.get(SCHEMA_KEY, LEGACY_SCHEMA) != DIRECT_PREROLL_SCHEMA
            ):
                continue
            ...
            for pole_data in armature.data.bones:
                ...
                if lower is None or pole is None or not bool(getattr(pole, "select", False)):
                    continue
                ...
                if (
                    ...
                    continue
                segments.append((pole_data.name, joint_world, pole_world))
        except (AttributeError, ReferenceError, RuntimeError, TypeError, ValueError):
            continue
    segments.sort(key=lambda item: item[0])
    return tuple((joint, pole) for _name, joint, pole in segments)


def _pole_guide_line_vertices(segments, view_direction=(0.0, 0.0, -1.0), view_up=(0.0, 1.0, 0.0)):
    """Build one view-facing shaft-and-head line batch for selected Poles.

    The arrowhead base is constructed from a vector perpendicular to the
    joint-to-Pole shaft.  Keeping this as pure geometry makes the guide
    testable and avoids helper objects, bones, constraints, or saved data.
    """
    ...
    if direction.length <= EPSILON or any(not math.isfinite(float(value)) for value in direction):
        direction = Vector((0.0, 0.0, -1.0))
    else:
        direction.normalize()
    if up.length <= EPSILON or any(not math.isfinite(float(value)) for value in up):
        up = Vector((0.0, 1.0, 0.0))
    else:
        up.normalize()

    vertices = []
    for joint_value, pole_value in segments:
        ...
        length = shaft.length
        axis = shaft / length
        lateral = axis.cross(direction)
        if lateral.length <= EPSILON:
            lateral = up - axis * up.dot(axis)
        if lateral.length <= EPSILON:
            cardinal = min(
                (Vector((1.0, 0.0, 0.0)), Vector((0.0, 1.0, 0.0)), Vector((0.0, 0.0, 1.0))),
                key=lambda candidate: abs(axis.dot(candidate)),
            )
            lateral = cardinal - axis * cardinal.dot(axis)
        if lateral.length <= EPSILON:
            continue
        lateral.normalize()
        # Reproject after normalization so accumulated floating-point error
        # cannot tilt the base away from a true shaft-perpendicular line.
        lateral -= axis * lateral.dot(axis)
        if lateral.length <= EPSILON:
            continue
        lateral.normalize()
        head_length = length * 0.15
        half_width = head_length * 0.475
        base_center = pole - axis * head_length
        left = base_center + lateral * half_width
        right = base_center - lateral * half_width
        vertices.extend((joint, pole, pole, left, left, right, right, pole))
...
def _draw_direct_pole_guides():
    ...
    try:
        ...
        region_data = getattr(context, "region_data", None) or getattr(space, "region_3d", None)
        view_rotation = getattr(region_data, "view_rotation", None)
        if view_rotation is None:
            view_direction = Vector((0.0, 0.0, -1.0))
            view_up = Vector((0.0, 1.0, 0.0))
        else:
            view_direction = view_rotation @ Vector((0.0, 0.0, -1.0))
            view_up = view_rotation @ Vector((0.0, 1.0, 0.0))
        vertices = _pole_guide_line_vertices(segments, view_direction, view_up)
        if not vertices:
            return
        batch = batch_for_shader(_POLE_GUIDE_SHADER, "LINES", {"pos": vertices})
        ...
        try:
            ...
            _POLE_GUIDE_SHADER.uniform_float("color", POLE_GUIDE_COLOR)
...
def _widget_geometry(kind, *, schema=CURRENT_SCHEMA):
    ...
    """
    if schema == LEGACY_SCHEMA and kind not in {"FINGER", "SHOULDER"}:
        if kind == "HAND":
...
def _validate_inventory(armature):
    ...
    if not armature_id:
        ...
        return {"armature_id": "", "schema": 0, "rigs": {}, "records": [], "bones": [], "master": None, "master_records": [], "source_widgets": None}
    ...
    if schema not in {LEGACY_SCHEMA, ENHANCED_SCHEMA, ROLL_DECOUPLED_SCHEMA, DIRECT_PREROLL_SCHEMA}:
        raise LimbIKError("The Limb IK schema version is unsupported.")
    if not generated and not records:
        if DIRECT_REST_KEY in armature.data or SOURCE_WIDGETS_KEY in armature.data:
            raise LimbIKError("Orphaned Limb IK Rest or source-control display data was found on this Armature.")
        return {"armature_id": armature_id, "schema": schema, "rigs": {}, "records": [], "bones": [], "master": None, "master_records": [], "source_widgets": None}
    ...
    for pose_bone, constraint, record in records:
        ...
        if record["role"] == "END_ROTATION":
            ...
            if bool(constraint.mute) != auto_value:
                raise LimbIKError(
                    f"Owned end rotation '{constraint.name}' does not match its Auto Align state."
                )
...
def _create_constraints_and_shapes(
    ...
    *,
    ...
    for plan in plans:
        ...
        runtime_constraints[plan.rig_id] = (ik_owner, ik, copy_rotation, *extras)
    ...
    for plan in plans:
        ...
        if schema == LEGACY_SCHEMA:
            ...
            set_shape(pole, widgets["POLE_ARROW"], chain_length * 0.11)
        ...
        ik_owner, ik, copy_rotation, *extras = runtime_constraints[plan.rig_id]
...
def _removal_resources(context, armature, inventory):
    ...
    if any(kind == "LEG" for kind, _side in inventory["rigs"]):
        if _is_direct_preroll_schema(inventory["schema"]):
            for key, rig in inventory["rigs"].items():
                ...
                if kind != "LEG":
                    continue
                shape = armature.pose.bones[rig["target"].name].custom_shape
                shape_kind = shape.get(KIND_KEY) if _owned(shape, armature_id, role="WIDGET") else None
                if shape_kind not in {"FOOT", f"FOOT_{side}"}:
                    raise LimbIKError(
                        f"Generated {side} Leg Foot Custom Shape ownership is invalid."
                    )
                direct_foot_widgets[key] = shape
    ...
    if _is_enhanced_schema(inventory["schema"]) and armature.pose.bones[MASTER_NAME].custom_shape is not widget_by_kind["MASTER"]:
    ...
    for (kind, side), rig in inventory["rigs"].items():
        ...
        if kind == "ARM":
            expected_target_shape = widget_by_kind["HAND"]
        elif _is_direct_preroll_schema(inventory["schema"]):
            expected_target_shape = direct_foot_widgets[(kind, side)]
        else:
            expected_target_shape = widget_by_kind["FOOT"]
        if target_pose.custom_shape is not expected_target_shape or pole_pose.custom_shape is not widget_by_kind[expected_pole]:
...
def _snapshot_owned_rig(armature, inventory):
    ...
    for (kind, side), rig in sorted(inventory["rigs"].items()):
        ...
        if kind == "LEG":
            shape = armature.pose.bones[target.name].custom_shape
            foot_widget_kinds[(kind, side)] = shape.get(KIND_KEY)
...
def _execute_build(operator, context, scope):
    ...
    if _is_direct_preroll_schema(schema):
        message = (
            ...
            "only Target + Pole controls were added per limb."
        )
    else:
...
class CHARACTERDESIGNER_OT_limb_ik_rebuild(Operator):
    ...
    def execute(self, context):
        ...
        try:
            ...
            # Auto Align is an animator-facing per-limb mode, not part of the
            # chain/direction planning inputs.  Carry it across every Rebuild,
            # including an intentional chain or Pole Direction change.
            prior_auto_align = {
            ...
            if previous_schema == CURRENT_SCHEMA:
                ...
                for plan in plans:
                    ...
                    if old is not None and old.chain.names == plan.chain.names and same_direction:
                        ...
                        preserved.append(replace(old, rig_id=uuid.uuid4().hex))
                    else:
...
def _solve_auto_align_target(armature, inventory, rig, target, solver, free_end, end_rotation):
    ...
    if local_owner_orient:
        ...
        result = Matrix.LocRotScale(
            ...
            free_basis.to_quaternion().normalized(),
            old_basis.to_scale(),
        )
...
def _set_auto_align_selected_target(context, armature, settings, enabled=None):
    ...
    Auto mode mutes only the owned END_ROTATION constraint, so Hand/Foot
    orientation follows the live solved chain without a dependency-cycle,
    polling handler, or transform writes.  Returning to Manual first aligns
    the Target while Auto is still live, then unmutes without a pose pop.
    """
    ...
    end_world_before = armature.matrix_world @ end.matrix
    try:
        if desired_enabled:
...
class CHARACTERDESIGNER_OT_limb_ik_auto_align_target(Operator):
    ...
    bl_description = "Toggle continuous natural Hand/Foot alignment; turn it off for manual Target rotation"
    bl_options = {"REGISTER", "UNDO"}
    ...
    def execute(self, context):
        ...
        try:
            ...
            target_name, enabled, changed = _set_auto_align_selected_target(
                context,
                armature,
                settings,
                desired,
            )
            kind, side = SELECTED_LIMBS[settings.selected_limb]
            state = "Auto" if enabled else "Manual"
            message = f"{side} {kind.title()} Target '{target_name}' is in {state} rotation mode."
            if not changed:
...
def _selected_auto_align_ui_state(context, settings):
    """Cheap, tolerant panel query; strict validation stays in the operator."""

    # This state is saved on the generated Target, whereas the analysis
    # PointerProperty is intentionally transient/SKIP_SAVE.  Resolve the
    # active rig directly so reopening a file never disables or misroutes the
    # toggle before the next Analyze action.
    armature = getattr(context, "object", None) if settings is not None else None
    kind, side = SELECTED_LIMBS.get(getattr(settings, "selected_limb", ""), ("", ""))
    if (
        armature is None
    ...
    try:
        ...
        targets = [
            bone
            for bone in armature.data.bones
            if _owned(bone, armature_id, role=role)
            and bone.get(KIND_KEY) == kind
            and bone.get(SIDE_KEY) == side
        ]
        if len(targets) != 1:
            return False, False
        value = targets[0].get(AUTO_ALIGN_KEY, False)
        return type(value) is bool, bool(value) if type(value) is bool else False
    except (AttributeError, ReferenceError, RuntimeError, TypeError, ValueError):
        return False, False
...
class CHARACTERDESIGNER_PT_limb_ik(Panel):
    ...
    def draw(self, context):
        ...
        auto_available, auto_enabled = _selected_auto_align_ui_state(context, settings)
        auto_row = layout.row()
