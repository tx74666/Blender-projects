...
def _retarget_custom_shape_frame(pose_bone, transform):
    ...
    next_local = next_transform.matrix.inverted_safe() @ current_frame
    if not _matrix_is_finite(next_local):
        raise LimbIKError(
            f"Control '{pose_bone.name}' cannot derive a finite Auto Align display frame."
        )
    translation = next_local.to_translation()
    rotation = next_local.to_quaternion().normalized().to_euler(
        "XYZ",
