...
Version 0.29.5 adds the read-only **Direct Pre-Roll Lab** under Limb IK. After
Analyze, choose one limb and run **Check Selected Limb**. The result reports the
modeled Rest-plane offset, the Rest-joint shift required to put the bend on the
configured Pole plane, and coherent upper/lower EditBone rolls for that proposed
frame. The check writes only session state and can run while the existing
schema-3 MCH/ORI rig remains present; it never edits or removes bones.

The Lab intentionally does not offer Apply yet. Blender skinning uses
`pose matrix * inverse Rest matrix`, so changing EditBone roll by itself is
cancelled by the corresponding Rest inverse. Re-planing the Rest joint can make
one calibrated Direct IK pose start clean, but moving the Pole consumes the same
axial degree of freedom again. Direct mode is therefore only a candidate for a
narrow, mostly fixed-Pole range; MCH/ORI remains the production default.
...
1. Make the intended source Armature active in Object or Pose Mode, open the
   **Rig** page, and click **Analyze Rig**. Choose Left/Right Arm or Leg in the
   ...
   defaults are elbow-behind (`+Y`) and knee-forward (`-Y`).
...
The collapsed **Direct Pre-Roll Lab** is also read-only. Its absolute Roll U/L
values describe the frame *after* the reported Rest-joint re-plane; entering
those Roll values alone does not repair skin twist. A result of **Aligned** means
only that the starting Rest plane is a plausible Direct IK candidate, not that
every Target/Pole pose will remain twist-free.
