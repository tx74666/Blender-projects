...
def _validate_inventory(armature):
    ...
    if schema not in {LEGACY_SCHEMA, ENHANCED_SCHEMA, ROLL_DECOUPLED_SCHEMA}:
...
def _build_plans(context, armature, plans, *, schema=CURRENT_SCHEMA, desired_source_pose=None):
    ...
    try:
        ...
        if _is_enhanced_schema(schema) and armature.data.get(SCHEMA_KEY) != schema:
...
def _restore_owned_snapshot(context, armature, snapshot):
    ...
    if _is_enhanced_schema(snapshot["schema"]):
...
class CHARACTERDESIGNER_OT_limb_ik_rebuild(Operator):
    ...
    def execute(self, context):
        ...
        try:
            ...
            context.view_layer.update()
            for plan in plans:
                if plan.mechanism_pose:
                    # Same-schema, unchanged-direction plans carry the old MCH
                    # input basis and intentionally take the exact refresh path.
                    continue
        ...
        try:
            ...
            if result["status"] == "UNSTABLE":
                ...
                message = (
                    ...
                    f"a {result['joint_shift']:.5g} m Rest-joint re-plane is required. No bones changed."
                )
                level = "WARNING"
            _set_status(settings, level, message)
            self.report({"WARNING" if level == "WARNING" else "INFO"}, message)
...
class CHARACTERDESIGNER_PT_limb_ik_direct_preroll(Panel):
    ...
    def draw(self, context):
        ...
        layout.label(text=f"Joint Shift  {float(result.get('joint_shift', 0.0)):.5g} m")
        layout.label(
            text=(
