...
def _read_records(obj):
    ...
    try:
        ...
        if (record["version"] != 1 or not isinstance(record["chains"], list) or not record["source_id"]
        ...
        for chain in record["chains"]:
            if (not isinstance(chain["signature"], str) or not isinstance(chain["bones"], list)
                    ...
                    or not isinstance(chain["vertices"], list) or not chain["layers"]
    ...
    try:
        ...
        for plan in plans:
            ...
            for earlier in result:
                overlap = vertices & set(earlier["vertices"])
                if overlap and not (overlap.issubset(layers[0]) and overlap.issubset(earlier["layers"][0])):
            ...
            if any(len(point) != 3 or not _finite(point) for point in centers):
            ...
            if not signature or any(item["signature"] == signature for item in result):
                raise HairBonesRigError("Hair bands have duplicate or invalid identities.")
            count = min(int(bone_count), len(centers) - 1)
            distances = [0.0]
            ...
            result.append({"signature": signature, "layers": layers, "centers": centers,
                           "vertices": tuple(sorted(vertices)), "count": count,
                           "requested_count": int(bone_count), "distances": tuple(distances),
...
def _weight_plan(obj, armature, plans, parent_bone, first_binding, old_states):
    ...
    for plan in plans:
        count = plan["count"]
        for layer, distance in zip(plan["layers"], plan["distances"]):
            root_blend = min(1.0, distance / plan["distances"][-1] * count * 2.0)
            if root_blend < 1.0:
                # Root sections are anchored to the common attachment. This
                # gives welded strand roots one consistent weight assignment.
                ...
                continue
            value = max(0.0, min(count - 1.0, distance / plan["distances"][-1] * count - 0.5))
...
def build_hair_bones(context, obj, plans, *, bone_count=4, armature=None, parent_bone=""):
    """Build/select independent FK chains and commit the entire batch atomically.
    ...
    for plan in plans:
        ...
        if old:
            if (old["count"] != plan["count"] or old["requested_count"] != plan["requested_count"]
                ...
                raise HairBonesRigError("This band's bone count changed; undo its chain before rebuilding.")
        ...
        for earlier in owned.values():
            ...
            if overlap and not (overlap.issubset(plan["layers"][0]) and overlap.issubset(earlier["layers"][0])):
        ...
        chain_index = len(owned) + len(new) + 1
        names = tuple(f"Hair {label} {chain_index:02d}.{index + 1:02d}" for index in range(plan["count"]))
    ...
    try:
        ...
        for plan in new:
            updated["chains"].append({"signature": plan["signature"], "bones": list(plan["names"]),
