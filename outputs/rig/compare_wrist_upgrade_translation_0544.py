...
def main():
    ...
    try:
        ...
        for label, parent_trs, rig_trs in cases:
                ...
                for offset in ((0, 0, 0), (.31, -.27, .42)):
                    ...
                    for axis in range(3):
                        ...
                        if TRANSLATIONS_ONLY:
                            ...
                            restore = (legacy.subtarget, legacy.owner_space, legacy.target_space,
                                       legacy.space_object, legacy.space_subtarget, legacy.mix_mode)
                            legacy.subtarget = target.name
                            legacy.owner_space = legacy.target_space = 'LOCAL'
                            legacy.space_object = None
                            legacy.space_subtarget = ''
                            legacy.mix_mode = 'AFTER'
                            d.update(rig)
                            ...
                            entry['translations'][-1]['upgrade_vs_legacy_hand_delta_error'] = (shift['hand'] - old_shift).length
                            target.matrix_basis = base
                            (legacy.subtarget, legacy.owner_space, legacy.target_space,
                             legacy.space_object, legacy.space_subtarget, legacy.mix_mode) = restore
                            d.update(rig)
            ...
            if TRANSLATIONS_ONLY:
                case['summary']['max_G_upgrade_vs_legacy_delta_error'] = max(r['upgrade_vs_legacy_hand_delta_error'] for r in translations)
    ...
    name = 'wrist_upgrade_0544_translation_comparison.json'
