"""Independent Child-Of prototype stress test, entirely in a disposable file copy."""
import bpy
import hashlib
...
TRANSLATIONS_ONLY = '--translations-only' in sys.argv
...
def main():
    ...
    try:
        ...
        for side in ('L', 'R'):
            ...
            for con in hand.constraints:
                if 'AUTO_OFFSET_ROTATION' in con.name:
                    con.mute = True
            target.rotation_euler = (0, 0, 0)
            d.update(rig)
            parent = target.parent.matrix if target.parent else Matrix.Identity(4)
            neutral = parent.inverted() @ target.matrix
            con = hand.constraints.new('CHILD_OF')
            con.name = 'Independent Parent Delta Prototype'
            con.target, con.subtarget = rig, target.name
            for field in ('use_location_x', 'use_location_y', 'use_location_z', 'use_scale_x', 'use_scale_y', 'use_scale_z'):
                setattr(con, field, False)
            con.owner_space = con.target_space = 'CUSTOM'
            con.space_object = rig
            con.space_subtarget = target.parent.name if target.parent else ''
            con.inverse_matrix = neutral.to_quaternion().inverted().to_matrix().to_4x4()
        d.update(rig)
    ...
    name = 'wrist_parent_delta_0544_translation_comparison.json' if TRANSLATIONS_ONLY else 'wrist_parent_delta_0544_stress.json'
